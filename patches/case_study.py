...
def full_sort_scores(uid_series, model, test_data, device=None):
    ...
    if not test_data.is_sequential:
        ...
        # patch to avoid None in history_row and history_col
        # replace with zero length tensor instead
        
        # history_row = torch.cat(
        #     [torch.full_like(hist_iid, i) for i, hist_iid in enumerate(history_item)]
        # )
        history_row = torch.cat([
            torch.full((len(hist_iid),), i, dtype=torch.long, device=device)
            if (hist_iid is not None and len(hist_iid) > 0) 
            else torch.tensor([], dtype=torch.long, device=device)
            for i, hist_iid in enumerate(history_item)
        ])

        # history_col = torch.cat(list(history_item))
        history_col = torch.cat([
        torch.tensor(hist_iid, dtype=torch.long, device=device)
        if (hist_iid is not None and len(hist_iid) > 0)
        else torch.tensor([], dtype=torch.long, device=device)
        for hist_iid in history_item
        ])
